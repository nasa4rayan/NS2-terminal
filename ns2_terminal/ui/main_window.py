...
============================
Premium glassmorphism main window with sidebar, tab bar, particle overlay,
command palette, custom scrollbars, and full keyboard shortcut integration.
"""
...
from PyQt6.QtGui import (
    QColor, QKeySequence, QShortcut, QIcon, QPainter,
...
class BackgroundCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Subtle vertical gradient depth — using theme's bg → secondary_glow
        grad = QLinearGradient(0, 0, 0, max(1, h))
        top = QColor(theme.background)
        bot = QColor(theme.secondary_glow)
        top.setAlphaF(0.75 * self._opacity)
        bot.setAlphaF(0.92 * self._opacity)
        ...
        # Breathing center glow (subtle, primary-only)
        pulse = 0.5
        if config.animations_enabled:
            pulse = 1.0 + 0.12 * (0.5 + 0.5 * math.sin(self._phase))

        glow_strength = (0.12 + 0.28 * self._glow_intensity) * pulse
        glow_alpha = int(255 * min(0.10, glow_strength))

        radial = QRadialGradient(w * 0.5, h * 0.38, max(240.0, min(w, h) * 0.72))
        c0 = QColor(theme.primary)
        c0.setAlpha(glow_alpha)
        c1 = QColor(theme.primary)
        c1.setAlpha(int(glow_alpha * 0.35))
        radial.setColorAt(0.0, c0)
        radial.setColorAt(0.40, c1)
...
# ═══════════════════════════════════════════════════════════════════════
# GLOBAL QSS — scrollbars, focus states, common widgets
# ═══════════════════════════════════════════════════════════════════════

def _build_global_qss(theme) -> str:
    """Build global stylesheet with custom scrollbar and common tokens."""
    return f"""
        QMainWindow {{
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.particles.stackUnder(content_widget)
    ...
    def _apply_theme(self, _=None):
        ...
        self.setStyleSheet(_build_global_qss(theme))
        self.update()
    ...
    def _register_palette_commands(self):
        commands = [
            ...
            ("Theme: Midnight Steel", lambda: setattr(config, "theme", "Midnight Steel")),
